...
function addItem(event) {
  if(event.srcElement.id == 'addIncomeButton') {loadItemValue('income')};
  if(event.target.id == 'addExpenseButton') {loadItemValue('expense')};
  updateItems(); 
  updateBalance();
}
...
function showModal(id) {
  ...
  saveButton.addEventListener('click', () => {
    ...
    if (newName && newAmount > 0 && !isNaN(newAmount)) {
      ...
      modalText.innerText = 'Invalid input. Please enter a valid name and amount.';
    }
  });
...
function showItem(item) {
  ...
  document.querySelector(editButtonSelector).
  addEventListener('click', function() {editItem(itemId);});
...
function updateBalance() {
  let totalIncome = items.filter((item) => (item.type == 'income')).reduce((total, incomeItem) => total + incomeItem.amount, 0);
  let totalExpense = items.filter((item) => (item.type == 'expense')).reduce((total, expenseItem) => total + expenseItem.amount, 0);
  ...
  if (balance > 0) {availableBalanceElement.innerText = `Możesz jeszcze wydać ${balance} PLN`};
  if (balance == 0) {availableBalanceElement.innerText = "Bilans wynosi zero"}; 
  if (balance < 0) {availableBalanceElement.innerText = `Bilans jest ujemny. Jesteś na minusie ${Math.abs(balance)} PLN`};
}
